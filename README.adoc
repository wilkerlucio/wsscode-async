...
<1> ; similar to `go`, but will return a promise-channel, so in case this result gets
; passed to multiple readers, all of them will get the realized value or error.
;
; this alsos wraps the block in a try/catch, so if some exception happens it will get
; returned as the channel value
